angular.module('cloudberry.map')
    .controller('countMapCtrl', function($rootScope, $scope, $compile, cloudberry, cloudberryConfig, MapResultCache,
        ...
        $scope.dMap = function (result){
            ...
            function transition(polygon)
            {
              if(stateA.has(polygon.feature.properties.stateID ))
              {     
                    var color = getColor(polygon.feature.properties.count);
                    if(!polygon._path && polygon._layers){
                        
                        for(var key in polygon._layers)
                        {
                          var path = polygon._layers[key]._path;
                          
                          d3.select(path).transition()
                          .duration(2000)
                          .attr({stroke: '#92d1e1', fill: color})
                          
                          polygon._layers[key].options.fillColor = color;
                          polygon._layers[key].options.color = '#92d1e1';
                          
                        }
                      
                    }
                    else if(polygon._path){
                      d3.select(polygon._path).transition()
                      .duration(2000)
                      .attr({stroke: '#92d1e1' , fill: color})
                      polygon.options.fillColor = color;
                      polygon.options.color = "#92d1e1";
                    }
            ...
            function dynamicStyle(feature){
              if(stateA.has(feature.properties.stateID))
                  return {
                      fillColor: 'rgb(244, 185,66)',
                      weight: 1.5,
                      opacity: 1,
            ...
            function dynamicUpdate(){
                ...
                $scope.polygons[level+"Polygons"].setStyle(dynamicStyle);
                for (var key in $scope.polygons[level+"Polygons"]._layers)
                    transition($scope.polygons[level+"Polygons"]._layers[key])
            }
        ...
        $rootScope.$on("CallParentMethod", function(event,result){
          ...
          for(var i=0;i<result.data.length;i++)
            {
              set1.add(result.data[i]['geo_tag.stateID']);
            }
          $scope.dMap(set1);
